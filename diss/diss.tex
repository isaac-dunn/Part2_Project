...
\begin{enumerate}
	...
	\item $\exists k \in \mathbb{N}.\; i < k < n
	\,\wedge\, \textit{proc}(\pi_k) = p
...
\begin{figure}
	\begin{algorithmic}[1]
		...
		\Procedure{UpdateBacktrackSets}{$\pi,\, t_{p,s}$}
			...
			\If{$p \in \textit{enabled}(\sigma_{\text{dep}})$}
			add $p$ to $\textit{backtrack}(\sigma_{\text{dep}})$
			\Else {
				add all of $\textit{enabled}(\sigma_{\text{dep}})$
				to $\textit{backtrack}(\sigma_{\text{dep}})$
			} \EndIf
			\EndIf
		\EndProcedure
		\State
...
\begin{description}
	...
	maintaining two sets of processes: \textit{backtrack}
	and \textit{done}. The \textit{backtrack} set contains
	those processes whose next transition should be explored
	from the current state, and is initialised to any process
	whose next transition is enabled. Recursive calls to
	\textsc{Explore} may add more processes to \textit{backtrack}.
	The \textit{done} set contains those transitions
	in \textit{backtrack} that have already been explored.
	...
	\item[Lines 10--13] If there is some transition that needs
	exploring, \textsc{Explore} is recursively called,
	and the process of that transition is added to \textit{done}.
	
	\item[Line 15] The textsc{UpdateBacktrackSets} procedure
	can use the information that a given transition, $t_{p,s}$ can be
	executed in $\sigma$ to add necessary backtracking points.
	
	\item[Line 16] The set $D$ contains the indices $i$ of the
	...
	\item[Line 17] If there are no transitions which have
	...
	\item[Line 18] If there are transitions in $\pi$
	which have a race condition with $t_{p,s}$, then
	it is sufficient to add a backtracking point at
	the state immediately before
	the most recent of these; the necessary backtracking
	points for the earlier states will be added
	by later (or previous) calls of \textsc{Explore}.
	
	\item[Lines 19--20] TODO: write this, but also
	sort out what \textit{enabled(state)} means (cf.
	algorithm). Also just before this subsection,
	there is a reference to $n$, which is undefined. 
