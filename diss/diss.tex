...
See the \textbf{APPENDIX} for a side-by-side of
the OCaml code and the pseudocode.
...
\begin{figure}
	\begin{algorithmic}[1]
		...
		\Let{\textit{one\_thread\_can\_advance}}{false}
		\Let{\textit{no\_stopped\_threads\_can\_progress}}{true}
		\Let{\textit{recursively\_deadlock\_free}}{true}
		\ForAll{$p \in \mathcal{P}$}
		\If{$\textit{next}(\sigma, p) \in \textit{enabled}(\sigma)$}
			\State \textit{one\_thread\_can\_advance} := true;
			\Let{$(\textit{result\_ef}, \textit{result\_df})$}
				{\Call{Check}{$\sigma_0, \pi.\textit{next}(\sigma, p)$}}
			\State \textit{error\_free} := \textit{error\_free}
				\&\& \textit{result\_ef};
			\State \textit{recursively\_deadlock\_free}
			:= \textit{recursively\_deadlock\_free}
			\&\& \textit{result\_df}
		\Else
			\If{$\textit{err}(l(p))$}{ \textit{error\_free} := false};
			\EndIf
			\If{DEADLOCK CASE}{ \textit{no\_stopped\_threads\_can\_progress} := false}
			\EndIf
		\EndIf
		\EndFor
		\Let{\textit{deadlock\_free}}{\textit{recursively\_deadlock\_free} \&\&
			(\textit{one\_thread\_can\_advance} $\lVert$ \textit{no\_stopped\_threads\_can\_progress})}
