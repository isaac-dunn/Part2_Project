...
\begin{description}
	...
	I wrote a function which applied only the first
	$n$ transitions in the sequence, for use later on.
	To get $\sigma$, this function is simply passed
	$|\pi|$, the length of the transition sequence.
	...
	in \textbf{CLOCK VECTOR SECTION}.

	\textbf{TODO: how do we know if a transition
	...
	\item[Lines 7--8] \textbf{Variable length arrays}

	\item[Lines 9--10] \textbf{\texttt{find\_p} function and \texttt{whileloop}}

	\item[Line 11] \textbf{Variable length arrays}

	\item[Line 12] \textbf{Recursive call}

	\item[Line 13] \textbf{Mention returning of result? Or point to
	error and deadlock detecting section?}
