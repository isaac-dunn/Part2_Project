...
possible transition does not interact with the transitions
in $T$.

Formally, a a subset of transitions $T \subseteq \mathcal{T}$
from a state $\sigma_1$
is called \emph{persistent in $\sigma_1$} if and only if
for all non-empty sequences of transitions
\[
	\sigma_1 \xrightarrow{t_1} \sigma_2 \xrightarrow{t_2} \ldots
	\xrightarrow{t_{n-1}} \sigma_n,
\]
if for all $1 <= i <= n$, $t_i \not \in T$ then $t_n$ is
