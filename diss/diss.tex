...
In Figure~\ref{fig:repeated-access-trans},
the stateful simple model-checker can be
seen to perform consistently well. Since
each access to a shared object is only a read,
performing any two reads in either order results
in the same state, resulting in many repeated
states being identified, explaining the good
performance.
In the other, more realistic examples, particularly
Figure~\ref{fig:prod-cons-trans},
the performance is again considerably
better than the simple algorithm.
...
\subsubsection{Stateful Simple Model Checking}
The cost of the hash-table operations
necessary for stateful checking means that
the performance of the simple stateful
algorithm is not quite as good as the
reductions in explored transitions might
suggest---for example, in Figure~\ref{fig:indexer-time},
the stateful algorithm takes longer to
complete than the simple algorithm,
despite exploring fewer transitions.
...
performing model checking statefully
dominate the costs of the hash-table
operations.
