...
\begin{itemize}
	\item As mentioned, is a functor from Exp to Sto
	\item Uses list implementation; other, more efficient are possible
	\item Talk through functions? (remember too much detail is irrelevant)
\end{itemize}
\subsubsection{The Thread Module}
	\begin{itemize}
		\item Main PL code here, implementing operational semantics
		\item Next step function. Explain why enabled hacked on.
		\item Explain derivation of next transition from next step.
			Could this be a functor instead?
		\item Why is \texttt{check local} here?
	\end{itemize}
\subsubsection{The Program Module}
	\begin{itemize}
		\item Functor. Not much to it, really: applying steps and
			transitions, and partial-order traces.
	\end{itemize}
\subsubsection{Testing}
	\begin{itemize}
		\item Tests to verify next step function
		\item Tests to verify next transition function
		\item Nothing too rigorous: just enough to convince
			me that it is probably okay.
		\item Even if not correct, should be easy to fix; priority
			is the more interesting part of the project
	\end{itemize}
