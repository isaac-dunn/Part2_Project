...
executions of under 0.04 seconds. The precision of measurements seems
...
\begin{itemize}
	\item small benefit, getting slightly better as
	more threads introduced but especially as
	more threads per object, and also more
	work per thread
	\item this is because the accesses do nothing, so
	if thread 0 and thread 1 both perform one access,
	then the result is the same, which isn't identified
	by model checking as it is assumed that if they access
	the same object, they are dependent, but they are
	practically independent.
\end{itemize}

\subsubsection{Dynamic Partial-Order Reduction}
\begin{itemize}
	\item In example 1, performs consistently
	very well for 1 access, but less well
	for 3 accesses.
	\item When each thread independent,
	basically free, because no race
	conditions so no backtracking points
	so only one execution.
	\item Otherwise, introduction
	of a thread either goes horizontally or
	multiplicative increase.
	\item Horizontal because new independent
	thread makes no difference: no new
	race condition, backtracking points, cost.
	\item Multiplicative increase because
	dependent, so could introduce race
	condition, so must check this extra possibility
	at each state.
	\item Increase not as steep as for
	simple though, because the independence
	between threads sharing objects can be exploited.
	\item 
	\item in prod-cons, does consistently well,
	because the threads are largely independent (only
	when they are at same item out of eight
	do they access the same object)
	\item indexer and fs are examples specifically
	designed to show off DPOR, so it does very well,
	since it turns out in both cases that the threads
	are independent until a particular number, beyond
	which performance gets worse quite quickly.
	\item aside that this suggests DPOR correctly
	implemented
\end{itemize}

\subsubsection{Stateful Dynamic Partial-Order Reduction}
\begin{itemize}
	\item SDPOR either offers no improvement on DPOR,
	or more often does a fair bit worse
	\item Backtracking points added too conservatively
	\item Algorithm or misimplementation or examples?
	\item Could be algorithm
	\item Could be I misunderstood and implemented a
	correct but more conservative similar algorithm.
	Inclusion of bounded buffer in their paper suggests
	it might be this, although they used more than one
	producer and consumer.
	\item Could be just unlucky on the examples I chose
	\item What examples would SDPOR be good at?
\end{itemize}

\subsubsection{Sleep Sets}
\begin{itemize}
	\item For simple, just nicely decreases the
	multiplicative increase by preventing
	re-exploration of areas of state space.
	\item For DPOR, makes no difference
	for ex1, and little for indexer, but
	big benefit for FS and prod-cons
	\item When does sleep work well?
	\item mention DPOR/sleep interaction
	mentioned in popl'05
\end{itemize}

\subsubsection{Static Partial-Order Reduction}
\begin{itemize}
	\item When all independent, does perfectly as
	size of each persistent set is 1
	\item otherwise, offers small benefit compared
	to simple or simple w/ sleep, which
	gets \emph{smaller} as threads/object increases.
	\item does seem to offer more benefit when
	each thread has more operations, too
	\item in reality, my algorithm is shit, and
	threads are unlikely to be partitioned into
	sets accessing disjoints sets of objects
	\item this is why it does not appear on
	later examples, as this condition is not
	met, so it offers literally no benefit
	\item main lesson learnt is how difficult
	spor is
\end{itemize}

\subsection{Efficiency of Implementations}
\subsubsection{Simple Model Checker}
\begin{itemize}
	\item pretty unremarkable, just constant work
	(per thread)
	per transition explored, so proportional to
	state space explored, so exponential
	\item size of state space by far dominates cost
\end{itemize}

\subsubsection{Stateful Simple Model Checking}
\begin{itemize}
	\item Although offers small advantage in terms
	of state space reduction, in practice seems to
	be counteracted by hash table operations, so that
	it ends up being the same as simple or slightly
	worse.
	\item notably, the one case where is does do better
	is (4, 3) -- perhaps in bigger, more industrial
	problems, it would offer more of an advantage.
	\item see note on SDPOR section about more
	efficient stateful checking
\end{itemize}

\subsubsection{Dynamic Partial-Order Reduction}
\begin{itemize}
	\item constant work per thread per transition,
	but again as threads increase it is by far
	the state explosion problem, not this work,
	that dominates
	\item so expect time basically proportional
	to transitions, so fact that data bear this
	out is sign of correct implementation
\end{itemize}

\subsubsection{SDPOR}
\begin{itemize}
	\item Much worse than DPOR, and even than
	SDPOR transitions. Clearest in
	indexer and FS, where state space is same,
	but time gets exponentially worse, due to
	costs of storing states
	\item in paper, proposed a way of better
	storing states, because states are big,
	but I did not implement this, explaining
...
\begin{itemize}
	\item Simple:
	\item overhead slightly increases
	multiplicative increase, but benefits still great
	\item DPOR:
	\item in ex1, transitions same, but
	there is a time overhead for performing
	the sleep sets computation
	\item this is not evident in later
	examples, though, when the benefits
	far outweigh this small overhead
\end{itemize}

\subsubsection{Static Partial-Order Reduction}
\begin{itemize}
	\item no noticeable overhead (by choice)
\end{itemize}
