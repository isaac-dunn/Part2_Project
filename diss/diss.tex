...
the number of different permutations of $kn$ letters,
...
\begin{figure}
	...
	\end{algorithmic}
	\caption{Algorithm deciding whether a transition system is error-free.}
...
\begin{figure}
	...
	\end{algorithmic}
	\caption{Algorithm deciding whether a transition system is deadlock-free.}
...
\begin{understandinglist}
	...
	system;
	\item that the simplest model checking algorithm
...
ML-like language, PL (short for Project Language), to avoid
...
Writing the simple model checker first allowed
me to be more prepared when implementing the DPOR
algorithm, and later provided a useful way of checking
its correctness. As with all model-checking
algorithms, I implemented the simple algorithm
as a functor which takes an implementation of
\textit{Program} as its input, and outputs
a structure that matches \textit{Checker}---in
other words, one that provides a model-checking
function.

Figure~\ref{fig:simple-imp-code} gives a high-level
description of the model-checking algorithm I
...
recursively deciding at each
state whether the state space
from the current state onwards is error-free
and deadlock-free.
Appendix~\ref{app:simple-code} shows the
correspondence between this pseudocode
and my actual OCaml function.
...
Each test ran the simple model checker
on a PL program, and
warned if the result was not as
...
to add locks to PL in a later ``increment'',
...
a transition, or it did not; there was
no way of specifying whether or not
the transition returned for the
given state was enabled in the
...
\begin{figure}
	...
	\end{algorithmic}
	\caption[The sleep sets algorithm.]
			{The simple model-checking algorithm with sleep sets.
				$H(\sigma)$ refers to the set of states stored
				in $H$ for $\sigma$.}
...
would not be as useful as you might at first think:
\begin{itemize}
	...
	with other,
	ideally independently developed,
...
is its widespread practical use, with considerable
success.
...
process went mostly to
plan, the incremental
strategy that I used
allowed me to overlook
an aspect of the system
design that became
issues in later
increments, as I mentioned
in section~\ref{sec:lock-difficulties}.
More time spent designing
the software before jumping
in with the implementation
(as is so tempting) would
have been beneficial.

It would have been interesting
to see how the DPOR algorithm
compared to full-strength
static partial-order reduction
algorithms, as opposed to the
poor one that I came up with.
Ideally, I should have
timetable in time to research
and implement one of these
algorithms, which would have
allowed a fuller analysis
of the benefits of DPOR.

I only realised that there was
something wrong with my
implementation of the SDPOR
algorithm when recording
data for use in the
evaluation chapter of this
dissertation, by which time
it was too late to fix.
Ideally, I would have
evaluated its performance
earlier on, rather than just
its soundness, to ensure that
my implementation was behaving
as expected.
